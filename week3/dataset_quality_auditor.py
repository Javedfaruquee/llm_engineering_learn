...
def build_report(reference_path: Path, synthetic_path: Path) -> str:
    ...
    if numeric_cols:
        ...
        # No literal pipes in header text: they would split cells and break the table.
        report.append("| Column | Ref mean | Syn mean | Abs delta mean | Ref std | Syn std | Abs delta std |")
        report.append("|---|---:|---:|---:|---:|---:|---:|")
...
def _validate_inputs(reference: Path, synthetic: Path) -> None:
    ...
    for label, path in (("Reference", reference), ("Synthetic", synthetic)):
