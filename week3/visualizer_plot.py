    ...
    def predict_tokens(self, prompt: str, max_tokens: int = 100) -> List[Dict]:
        ...
        for chunk in response:
            if not chunk.choices or not chunk.choices[0].logprobs:
                continue
            # A chunk can carry several tokens, each with its own logprobs.
            # Use the chosen token's own logprob rather than looking it up in
            # top_logprobs, which need not contain it when sampling is on.
...
def create_token_graph(model_name: str, predictions: List[Dict]) -> nx.DiGraph:
    ...
    # END follows the last generated token (or START if nothing was generated)
